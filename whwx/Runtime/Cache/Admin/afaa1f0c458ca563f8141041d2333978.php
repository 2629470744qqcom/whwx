...
<div id="wrapper">
    ...
    <div id="page-wrapper">
		<div id="page-inner">
	        <div class="row"> <h3 class="page-header">分类管理</h3></div>
<div class="row">
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-body">
            	<label><a href="<?php echo U('TourClassify/add');?>" class="btn btn-primary">添加</a></label>
                    ...
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                        	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                    ...
                                    <td><?php echo ((isset($list["link"]) && ($list["link"] !== ""))?($list["link"]):'无'); ?></td>
                                    <td>
                                        <input class="setSort" type="text" size="2" data-url="<?php echo U('TourClassify/setSort?id='.$list['id']);?>" value="<?php echo ($list["sort"]); ?>">
                                    </td>
                                    <td>
                                        <button class='btn <?php if(($list["status"]) == "1"): ?>btn-primary<?php else: ?>btn-default<?php endif; ?> btn-sm setStatus' type="button" data-url='<?php echo U("TourClassify/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>"><?php if(($list["status"]) == "1"): ?>启用<?php else: ?>禁用<?php endif; ?></button>
                                    </td>
                                    <td>
                                 	    <a class="btn-sm btn btn-default" href='<?php echo U("TourClassify/edit?id=".$list["id"]);?>'>修改</a>
                                 	    <button data-url='<?php echo U("TourClassify/del?id=".$list["id"]);?>' class="btn-sm btn btn-default delBtn" type="button">删除</button>
                                    </td>
