                    ...
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                        	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                    <td><?php echo ($i); ?></td>
                                    <td><?php if(($list["pid"]) > "0"): ?>&nbsp;|-&nbsp;&nbsp;<?php endif; echo ($list["name"]); ?></td>
                                    <td><?php echo ($list["value"]); ?></td>
                                    <td>
                                        <input class="setSort" type="text" size="2" data-url="<?php echo U('Wxmenu/setSort?id='.$list['id']);?>" value="<?php echo ($list["sort"]); ?>">
                                    </td>
                                    <td>
                                     	<button class="btn-sm btn btn-default editMenu" data-id='<?php echo ($list["id"]); ?>' >修改</button>
                                     	<button data-url='<?php echo U("Wxmenu/del?id=".$list["id"]);?>' class="btn-sm btn btn-default delMenu" type="button">删除</button>
                                    </td>
...
<div id="myModal" class="modal fade in" style="display: none;">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="<?php echo U('Wxmenu/add');?>" id="myform" class="form-horizontal">
...
$(function(){
    ...
    $('.addMenu').click(function(){
        ...
        $('#myModal').modal('show');
        $.get('<?php echo U("Wxmenu/add");?>', function(data){
    ...
    $('.editMenu').click(function(){
        ...
        $.get('<?php echo U("Wxmenu/edit");?>', {id: id}, function(data){
            ...
            $('#myform').attr('action', '<?php echo U("Wxmenu/edit");?>');
        });
        $('#myModal').modal('show');
    });
    //删除菜单
    ...
    $('.createMenu').click(function(){
        waiting();
        $.get('<?php echo U("Wxmenu/createMenu");?>', function(data){
    ...
    $('.syncMenu').click(function(){
        if(confirm("同步菜单会删除本地所有菜单。\n确定要同步吗？")){
            waiting();
            $.get('<?php echo U("Wxmenu/syncMenu");?>', function(data){
                complete(); showInfo(data.info);
                if(data.status == 1){
                    setTimeout(function(){
                        location.reload();
    ...
    $('.cancelMenu').click(function(){
        if(confirm("确定要撤回菜单吗？")){
            waiting();
            $.get('<?php echo U("Wxmenu/cancelMenu");?>', function(data){
                complete(); showInfo(data.info);
            });
        };
    });
