...
<div id="wrapper">
    ...
    <div id="page-wrapper">
		<div id="page-inner">
	        <div class="row"> <h3 class="page-header">优惠活动</h3></div>
<div class="row">
    <div class="col-md-12">
        <!-- Advanced Tables -->
        <div class="panel panel-default">
            <div class="panel-body">
              	<label><a class="btn btn-primary" href="<?php echo U('MerchantActive/add');?>">添加活动</a></label>
        				<form action='<?php echo U("MerchantActive/index");?>' class="form-inline text-right m-b-5" style="float:right;">
                    ...
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                          	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                    ...
                                    <td><?php echo (date('Y-m-d H:i:s', $list["start_time"])); ?>至<?php echo (date('Y-m-d H:i:s', $list["end_time"])); ?></td>
                                    <td><?php echo ($list["name"]); ?></td>
                                    <td><input class="setSort" type="text" size="2" data-url="<?php echo U('MerchantActive/setSort?id='.$list['id']);?>" value="<?php echo ($list["sort"]); ?>"></td>
                                    <td class="center">
                                      	<button class='btn <?php if(($list["status"]) == "1"): ?>btn-primary<?php else: ?>btn-default<?php endif; ?> btn-sm setStatus' type="button" data-url='<?php echo U("MerchantActive/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>"><?php if(($list["status"]) == "1"): ?>启用<?php else: ?>禁用<?php endif; ?></button>
                                    </td>
                                    <td class="center">
                                     	  <a href='<?php echo U("MerchantActive/edit?id=".$list["id"]);?>'><button class="btn-sm btn btn-default">修改</button></a>
                                      	<button data-url='<?php echo U("MerchantActive/del?id=".$list["id"]);?>' class="btn-sm btn btn-default delBtn" type="button">删除</button>
                                    </td>
