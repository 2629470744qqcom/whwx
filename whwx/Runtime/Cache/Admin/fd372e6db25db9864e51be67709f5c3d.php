...
<div id="wrapper">
    ...
    <div id="page-wrapper">
		<div id="page-inner">
	        <div class="row"> <h3 class="page-header">板块管理</h3></div>
<div class="row">
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-body">
            	<label><a href="<?php echo U('Plate/add');?>" class="btn btn-primary">添加</a></label>
                    ...
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                        	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                    ...
                                    <td><img src="<?php echo ($list["pic"]); ?>?imageView2/0/h/50/w/50" width="50" height="50"></td>
                                    <td><?php echo ($list["link"]); ?></td>
                                    <td>
                                        <?php switch($list["type"]): case "1": ?>物业服务<?php break;?>
                                            <?php case "2": ?>生活服务<?php break;?>
                                            <?php case "3": ?>邻里社交<?php break; endswitch;?>
                                    </td>
                                    <td>
                                        <input class="setSort" type="text" size="2" data-url="<?php echo U('Plate/setSort?id='.$list['id']);?>" value="<?php echo ($list["sort"]); ?>">
                                    </td>
                                    <td>
                                        <button class='btn <?php if(($list["status"]) == "1"): ?>btn-primary<?php else: ?>btn-default<?php endif; ?> btn-sm setStatus' type="button" data-url='<?php echo U("Plate/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>"><?php if(($list["status"]) == "1"): ?>启用<?php else: ?>禁用<?php endif; ?></button>
                                    </td>
                                    <td>
                                 	    <a class="btn-sm btn btn-default" href='<?php echo U("Plate/edit?id=".$list["id"]);?>'>修改</a>
                                 	    <button data-url='<?php echo U("Plate/del?id=".$list["id"]);?>' class="btn-sm btn btn-default delBtn" type="button">删除</button>
                                    </td>
