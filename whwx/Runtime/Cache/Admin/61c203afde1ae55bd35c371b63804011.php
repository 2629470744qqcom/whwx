...
<div id="wrapper">
    ...
    <div id="page-wrapper">
		<div id="page-inner">
	        <div class="row"> <h3 class="page-header">红黑榜</h3></div>
<div class="row">
    <div class="col-md-12">
        <!-- Advanced Tables -->
        <div class="panel panel-default">
            <div class="panel-body">
          		<label><a class="btn btn-primary" href="<?php echo U('BlackList/add');?>">添加</a></label>
				<form action='<?php echo U("BlackList/index");?>' class="form-inline text-right m-b-5" style="float:right;">
        	标题：<input type="text" name='name' value='<?php echo ($_GET['name']); ?>' class="form-control mr20" style="width: 150px;">
              状态：<select name='status' class="form-control mr20" style="width: 150px;">
                        <option value='-1'>全部</option>
                        <option value='1' <?php if(($_GET['status']) == "1"): ?>selected<?php endif; ?> >启用
                        </option>
                        <option value='0' <?php if(($_GET['status']) == "0"): ?>selected<?php endif; ?> >禁用
                        </option>
                    </select>
              类型：<select name='type' class="form-control mr20" style="width: 150px;">
                        <option value='-1'>全部</option>
                        <option value='1' <?php if(($_GET['type']) == "1"): ?>selected<?php endif; ?> >黑榜
                        </option>
                        <option value='0' <?php if(($_GET['type']) == "0"): ?>selected<?php endif; ?> >红榜
                        </option>
                    </select>
                    ...
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <tbody>
                        	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
                                    ...
                                    <td><?php echo ($list["zan"]); ?></td>
                                    <td class="center">
                                     	<button class='btn <?php if(($list["type"]) == "1"): ?>btn-default<?php else: ?>btn-danger<?php endif; ?> btn-sm' type="button"><?php if(($list["type"]) == "1"): ?>黑榜<?php else: ?>红榜<?php endif; ?></button>
                                    </td>
                                    <td><input class="setSort" type="text" size="2" data-url="<?php echo U('BlackList/setSort?id='.$list['id']);?>" value="<?php echo ($list["sort"]); ?>"></td>
                                    <td class="center">
                                     	<button class='btn <?php if(($list["status"]) == "1"): ?>btn-primary<?php else: ?>btn-default<?php endif; ?> btn-sm setStatus' type="button" data-url='<?php echo U("BlackList/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>"><?php if(($list["status"]) == "1"): ?>启用<?php else: ?>禁用<?php endif; ?></button>
                                    </td>
                                    <td class="center">
                                     	<a href='<?php echo U("BlackList/edit?id=".$list["id"]);?>'><button class="btn-sm btn btn-default">修改</button></a>
                                      	<button data-url='<?php echo U("BlackList/del?id=".$list["id"]);?>' class="btn-sm btn btn-default delBtn" type="button">删除</button>
                                    </td>
