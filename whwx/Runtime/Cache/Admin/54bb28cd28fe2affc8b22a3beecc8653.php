    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-body">
            	<label><a class="btn btn-primary" href="<?php echo U('Admins/add');?>">添加员工</a></label>
                <form action='<?php echo U("Admins/index");?>' class="form-inline text-right m-b-5 fr">
          员工名称：<input type="text" name='name' value='<?php echo ($_GET['name']); ?>' class="form-control mr20 w150">
          手机号码：<input type="text" name='tel' value='<?php echo ($_GET['tel']); ?>' class="form-control mr20 w150">
            所属组：<select name='gid' class="form-control mr20 w150">
                        <option value='-1'>全部</option>
                        <?php if(is_array($groupList)): $i = 0; $__LIST__ = $groupList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><option value='<?php echo ($group["id"]); ?>' <?php if(($_GET['gid']) == $group["id"]): endif; ?>><?php echo ($group["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                    ...
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                    ...
                                    <td>
                                       <?php if(($list["last_login_time"]) > "0"): echo (date('Y-m-d H:i', $list["last_login_time"])); else: ?>未登录<?php endif; ?>
                                    </td>
                                    <td><?php echo ((isset($list["last_login_ip"]) && ($list["last_login_ip"] !== ""))?($list["last_login_ip"]):'未登录'); ?></td>
                                    <td class="center">
                                     	<button class='btn <?php if(($list["status"]) == "1"): ?>btn-primary<?php else: ?>btn-default<?php endif; ?> btn-sm setStatus' type="button" data-url='<?php echo U("Admins/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>"><?php if(($list["status"]) == "1"): ?>启用<?php else: ?>禁用<?php endif; ?></button>
                                    </td>
                                    <td>
                                        <a class=" btn-sm btn btn-default" href='<?php echo U("Admins/edit?id=".$list["id"]);?>'>修改</a>
                                        <button data-url='<?php echo U("Admins/del?id=".$list["id"]);?>' class=" btn-sm btn btn-default delBtn" type="button">删除</button>
                                    </td>
