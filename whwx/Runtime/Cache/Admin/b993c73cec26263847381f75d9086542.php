...
<div id="wrapper">
    ...
    <div id="page-wrapper">
		<div id="page-inner">
	        <div class="row"> <h3 class="page-header">租售管理&nbsp;&nbsp;<small><?php if(($info["id"]) > ""): ?>/修改<?php else: ?>/添加<?php endif; ?></small></h3></div>
<div class="row">
		...
		<div class="panel-body">
			<form role="form"  id="myform" method="post" class="form-horizontal">
			    <div class="form-group">
			        ...
			        <div class="col-sm-4">
			        	<input class="form-control required" name="title" value="<?php echo ($info["title"]); ?>">
			        </div>
			    </div>
			    <div class="form-group">
				...
				<div class="form-group">
			        ...
			        <div class="col-sm-4">
			       		<div class="uploadify-div">
				       		<?php if(!empty($info["pics"])): if(is_array($info["pics"])): $i = 0; $__LIST__ = $info["pics"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pic): $mod = ($i % 2 );++$i;?><li>
								    	<img src="<?php echo ($pic); ?>" >
								    	<input type="hidden"  value="<?php echo ($pic); ?>" id='pics<?php echo ($i); ?>' name="pic[]">
			    ...
			    <div class="form-group">
			    	...
			    	<div class="col-sm-4">
			    		<input type="text" class="form-control required number" name='price' value="<?php echo ($info["price"]); ?>">
			    	</div>
			    	<small>单位(元)</small>
			    </div>
			     <div class="form-group">
			    	...
			    	<div class="col-sm-4">
			    		<input type="text" class="form-control required number" name='size' value="<?php echo ($info["size"]); ?>">
			    	</div>
			    ...
			    <div class="form-group">
					...
					<div class="col-sm-4">
						<select name='type' class="form-control mr20 selectReq">
							<option value='1' <?php if(($info["type"]) == "1"): ?>selected<?php endif; ?> >出租房屋</option>
                            <option value='2' <?php if(($info["type"]) == "2"): ?>selected<?php endif; ?> >出售房屋</option>
                            <option value='3' <?php if(($info["type"]) == "3"): ?>selected<?php endif; ?> >出租车位</option>
                            <option value='4' <?php if(($info["type"]) == "4"): ?>selected<?php endif; ?> >出售车位</option>
                            <option value='5' <?php if(($info["type"]) == "5"): ?>selected<?php endif; ?> >出租单间</option>
						</select>
					</div>
				</div> 
				<div <?php if(($info["type"] == 3) or ($info["type"] == 4)): ?>style="display:none;"<?php endif; ?>>
					<div class="form-group" id="home">
				    	<label  class="col-sm-3 control-label">房屋户型：<span class="c-red">*</span></label>
				    	<div class="col-sm-4">
				    		<div class="col-sm-4">
					    		<input type="text" class="form-control number required" style="width:90%; float:left;margin-right:3%" name='room' value="<?php echo ($info["room"]); ?>">
					    		<span style='width:5%;float:left;line-height:34px;'>室</span>
				    		</div>
				    		<div class="col-sm-4">
					    		<input type="text" class="form-control number required"  style="width:90%; float:left;margin-right:3%" name='hall' value="<?php echo ($info["hall"]); ?>">
					    		<span style='width:5%;float:left;line-height:34px;'>厅</span>
				    		</div>
				    		<div class="col-sm-4">
					    		<input type="text" class="form-control number required"  style="width:90%; float:left;margin-right:3%" name='toilet' value="<?php echo ($info["toilet"]); ?>">
				    ...
				    <div class="form-group" id="floor">
				    	...
				    	<div class="col-sm-4">
					    	<div class="col-sm-5">
						    	...
						    	<input type="text" class="form-control number required" style="width:70%;float:left;" name='floor_several' value="<?php echo ($info["floor_several"]); ?>">
						    	<span style='width:10%;float:left;line-height:34px;'>层</span>
						    </div>
					    	<div class="col-sm-5">
						    	<span style='width:10%;float:left;line-height:34px;'>共</span>
						    	<input type="text" class="form-control number required" style="width:70%;float:left;" name='floor_all' value="<?php echo ($info["floor_all"]); ?>">
						    	<span style='width:10%;float:left;line-height:34px;'>层</span>
					    	</div>
				    	</div>
				    </div>
				    <div class="form-group" id="house">
			            ...
			            <div class="col-sm-4">
			                <textarea rows="1" class="form-control required" name="house" maxlength="200"><?php echo ($info["house"]); ?></textarea>
			            </div>
				    </div>
			    </div>
				<div class="form-group">
		            ...
		            <div class="col-sm-4">
		                <textarea rows="1" class="form-control required" name="address" maxlength="200"><?php echo ($info["address"]); ?></textarea>
		            </div>
		        </div>
			    <div class="form-group">
		            <label class="col-sm-3 control-label">租售描述：<span class="c-red">*</span></label>
		            <div class="col-sm-4">
		                <textarea rows="3" class="form-control required" name="desc" maxlength="200"><?php echo ($info["desc"]); ?></textarea>
		            </div>
		        </div>
			    <div class="form-group">
					...
					<div class="col-sm-4">
			            ...
			            <label class="radio-inline">
			                <input type="radio" value="1" id="status1" name="status" <?php if(($info["status"]) != "0"): ?>checked<?php endif; ?>>正常
			            </label>
			            <label class="radio-inline">
			                <input type="radio" value="0" id="status2" name="status" <?php if(($info["status"]) == "0"): ?>checked<?php endif; ?>>删除
			            </label>
		            </div>
		        </div>
		        ...
		        <div class="form-group">
                    <div class="col-sm-offset-3 col-sm-5">
				        <button class="btn btn-primary" type="submit">保存</button>
				       	<a href="<?php echo U('RentalService/index');?>"><button class=" btn btn-default" type="button">返回</button></a>
			       	</div>	
		       	</div>
			</form>
		</div>
