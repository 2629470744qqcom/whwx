...
<div id="wrapper">
    ...
    <div id="page-wrapper">
		<div id="page-inner">
	        <div class="row"> <h3 class="page-header">轮播图</h3></div>
<div class="row">
    <div class="col-md-12">
        <!-- Advanced Tables -->
        <div class="panel panel-default">
            <div class="panel-body">
          		<label><a class="btn btn-primary" href="<?php echo U('MerchantSlide/add');?>">添加图片</a></label>
				<form action='<?php echo U("MerchantSlide/index");?>' class="form-inline text-right m-b-5" style="float:right;">
        ...
        幻灯片标题：<input type="text" name='name' value='<?php echo ($_GET['name']); ?>' class="form-control mr20" style="width: 150px;">
                    ...
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <tbody>
                        	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
                                    ...
                                    <td><?php if(!empty($list["url"])): ?><a href="<?php echo ($list["url"]); ?>" target="_blank">点击查看</a><?php else: ?> <a href="<?php echo C('site_url');?>/Wap/About/detail?type=2&id=<?php echo ($list["id"]); ?>" target="_blank">点击查看</a><?php endif; ?></td>
                                    <td>
                                        <input class="setSort" type="text" size="2" data-url="<?php echo U('MerchantSlide/setSort?id='.$list['id']);?>" value="<?php echo ($list["sort"]); ?>">
                                    </td>
                                    <td class="center">
                                     	<button class='btn <?php if(($list["status"]) == "1"): ?>btn-primary<?php else: ?>btn-default<?php endif; ?> btn-sm setStatus' type="button" data-url='<?php echo U("MerchantSlide/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>"><?php if(($list["status"]) == "1"): ?>启用<?php else: ?>禁用<?php endif; ?></button>
                                    </td>
                                    <td class="center">
                                     	<a href='<?php echo U("MerchantSlide/edit?id=".$list["id"]);?>' class="btn-sm btn btn-default">修改</a>
                                      	<button data-url='<?php echo U("MerchantSlide/del?id=".$list["id"]);?>' class="btn-sm btn btn-default delBtn" type="button">删除</button>
                                    </td>
