...
<div id="wrapper">
    ...
    <div id="page-wrapper">
		<div id="page-inner">
	        <div class="row"> <h3 class="page-header">星房推荐</h3></div>
    <div class="row">
        <div class="col-md-12"> 
            <div class="panel panel-default">
                <div class="panel-body">
                    <label><a class="btn btn-primary" href="<?php echo U('Recommend/add');?>">添加</a></label>
                    <form action='<?php echo U("Recommend/index");?>' class="form-inline text-right fr m-b-5">
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td>
                                            <input class="setSort" type="text" size="2" data-url="<?php echo U('Recommend/setSort?id='.$list['id']);?>" value="<?php echo ($list["sort"]); ?>">
                                        </td>
                                        <td>
                                            <?php switch($list["status"]): case "0": ?><button class=" btn btn-default btn-sm setStatus" type="button" data-url='<?php echo U("Recommend/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>">禁用</button><?php break;?>
                                                <?php case "1": ?><button class="btn btn-primary btn-sm setStatus" type="button" data-url='<?php echo U("Recommend/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>">启用</button><?php break; endswitch;?>
                                        </td>
                                        <td>
                                            <a class=" btn btn-default btn-sm" href="<?php echo U('Recommend/edit?id='.$list['id']);?>">修改</a>
                                            <button data-url='<?php echo U("Recommend/del?id=".$list["id"]);?>' class="btn-sm btn btn-default delBtn" type="button">删除</button>
                                        </td>
