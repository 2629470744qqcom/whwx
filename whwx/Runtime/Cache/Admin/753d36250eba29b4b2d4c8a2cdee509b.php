...
<div id="wrapper">
    ...
    <div id="page-wrapper">
		<div id="page-inner">
	        <div class="row"> <h3 class="page-header">预警管理</h3></div>
    <div class="row">
        <div class="col-md-12"> 
            <div class="panel panel-default">
                <div class="panel-body">
                    <form action='<?php echo U("Warn/index");?>' class="form-inline text-right fr m-b-5">
                  名称：<input type="text" name='name' value='<?php echo ($_GET['name']); ?>' class="form-control mr20 w150" >
                  状态：<select name='status' class="form-control mr20 w150" >
                            <option value='-1'>全部</option>
                            <option value='1' <?php if(($_GET['status']) == "1"): ?>selected<?php endif; ?> >未处理</option>
                            <option value='0' <?php if(($_GET['status']) == "0"): ?>selected<?php endif; ?> >已处理</option>
                        </select> 
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td>
                                        	<button class=" btn <?php if(($list["status"]) == "1"): ?>btn-danger<?php else: ?>btn-default<?php endif; ?>  btn-sm " type="button"><?php if(($list["status"]) == "1"): ?>未处理<?php else: ?>已处理<?php endif; ?></button>
                                        </td>
                                        <td>
                                        	<?php switch($list["type"]): case "1": ?><a href="<?php echo U('Repair/index?id='.$list['typeid']);?>" class="btn-sm btn btn-default" type="button">查看</a><?php break;?>
                                        		<?php case "2": ?><a href="<?php echo U('Booking/index?id='.$list['typeid']);?>" class="btn-sm btn btn-default" type="button">查看</a><?php break;?>
                                        		<?php case "3": ?><a href="<?php echo U('Complaint/index?id='.$list['typeid']);?>" class="btn-sm btn btn-default" type="button">查看</a><?php break; endswitch;?>
                                        </td>
  ...
  <script type="text/javascript">
   $(function(){
    $('.look').click(function(){
        ...
        if(type == 1){
            location.href = '<?php echo U("Repair/index");?>?id='+$(this).data('typeid');
        }
    })
   })
    </script>
