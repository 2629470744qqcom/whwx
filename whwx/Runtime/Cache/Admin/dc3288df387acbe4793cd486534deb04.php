...
<div id="wrapper">
    ...
    <div id="page-wrapper">
		<div id="page-inner">
	        <div class="row"><h3 class="page-header">有偿服务</h3></div>
<div class="row">
    <div class="col-md-12">
        <!-- Advanced Tables -->
        <div class="panel panel-default">
            <div class="panel-body">
          	    <label><a class="btn btn-primary" href="<?php echo U('RepairFixe/add');?>">添加有偿服务</a></label>
				<form action='<?php echo U("RepairFixe/index");?>' class="form-inline text-right m-b-5" style="float:right;">
          项目名称：<input type="text" name='name' value='<?php echo ($_GET['name']); ?>' class="form-control mr20" style="width: 150px;">
                    ...
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <tbody>
                        	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
                                    ...
                                    <th><?php echo ((isset($list["material"]) && ($list["material"] !== ""))?($list["material"]):'无'); ?></th>
                                    <td><?php echo ($list["price"]); ?>元</td>
                                    <td><input class="setSort" type="text" size="2" data-url="<?php echo U('RepairFixe/setSort?id='.$list['id']);?>" value="<?php echo ($list["sort"]); ?>"></td>
                                    <td class="center">
                                     	<button class='btn <?php if(($list["status"]) == "1"): ?>btn-primary<?php else: ?>btn-default<?php endif; ?> btn-sm setStatus' type="button" data-url='<?php echo U("RepairFixe/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>"><?php if(($list["status"]) == "1"): ?>启用<?php else: ?>禁用<?php endif; ?></button>
                                    </td>
                                    <td class="center">
                                     	<a href='<?php echo U("RepairFixe/edit?id=".$list["id"]);?>'><button class="btn-sm btn btn-default">修改</button></a>
                                      	<button data-url='<?php echo U("RepairFixe/del?id=".$list["id"]);?>' class="btn-sm btn btn-default delBtn" type="button">删除</button>
                                    </td>
