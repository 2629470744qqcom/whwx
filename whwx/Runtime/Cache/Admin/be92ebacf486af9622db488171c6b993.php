    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-body">
            	<label><a class="btn btn-primary" href="<?php echo U('Repairman/add');?>">添加维修员</a></label>
                <form action='<?php echo U("Repairman/index");?>' class="form-inline text-right m-b-5 fr">
        	维修员：<input type="text" name='name' value='<?php echo ($_GET['name']); ?>' class="form-control mr20 w150">
                    ...
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                    <td><?php echo ($i); ?></td>
                                    <td><?php echo ($list["name"]); if(($list["fid"]) <= "0"): ?>&nbsp;&nbsp;【WXBD#3#<?php echo ($list["phone"]); ?>#<?php echo ($list["id"]); ?>】<?php endif; ?></td>
                                    ...
                                    <td class="center">
                                    	<?php switch($list["status"]): case "-1": ?><button class='btn btn-danger btn-sm' type="button">已删除</button><?php break;?>	
                                    		<?php case "0": ?><button class='btn btn-default btn-sm setStatus' type="button" data-url='<?php echo U("Repairman/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>">禁用</button><?php break;?>
                                    		<?php case "1": ?><button class='btn btn-primary btn-sm setStatus' type="button" data-url='<?php echo U("Repairman/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>">启用</button><?php break;?>
                                    		<?php case "2": ?><button class='btn btn-primary btn-sm vette' data-id='<?php echo ($list["id"]); ?>' type="button">未审核</button><?php break; endswitch;?>
                                    </td>
                                    <td>
                                        <a class="btn-sm btn btn-default" href='<?php echo U("Repairman/edit?id=".$list["id"]);?>'>修改</a>
                                        <button data-url='<?php echo U("Repairman/del?id=".$list["id"]);?>' class="btn-sm btn btn-default delBtn" type="button">删除</button>
                                    </td>
...
<div id="myModal" class="modal fade in" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
	        <form action="<?php echo U('Repairman/vette');?>" id="myform" class="form-horizontal">
	...
	$(function(){
		...
		$('.save').click(function(){
		    ...
		    if(val == null){
		    	...
		    	waiting();
		    	$.post("<?php echo U('Repairman/vette');?>", $('#myModal #myform').serialize(),function(data){
		    		complete();
		    		showInfo(data.info);
		    		if(data.status == 1){
		    			obj.data('status', val);
