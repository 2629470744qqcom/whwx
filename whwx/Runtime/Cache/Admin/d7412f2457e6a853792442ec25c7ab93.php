...
<div id="wrapper">
    ...
    <div id="page-wrapper">
		<div id="page-inner">
	        <div class="row"><h3 class="page-header">部门管理</h3></div>
<div class="row">
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-body">
            	<label><a class="btn btn-primary" href="<?php echo U('Group/add');?>">添加部门</a></label>
                <form action='<?php echo U("Group/index");?>' class="form-inline text-right m-b-5 fr">
          部门名称：<input type="text" name='name' value='<?php echo ($_GET['name']); ?>' class="form-control mr20 w150">
                    ...
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                    ...
                                    <td class="center">
                                     	<button class='btn <?php if(($list["status"]) == "1"): ?>btn-primary<?php else: ?>btn-default<?php endif; ?> btn-sm setStatus' type="button" data-url='<?php echo U("Group/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>"><?php if(($list["status"]) == "1"): ?>启用<?php else: ?>禁用<?php endif; ?></button>
                                    </td>
                                    <td>
                                     	<a class="btn-sm btn btn-default" href='<?php echo U("Group/rule?id=".$list["id"]);?>'>权限</a>
                                        <a class="btn-sm btn btn-default" href='<?php echo U("Group/edit?id=".$list["id"]);?>'>修改</a>
                                        <?php if(($list["id"]) > "1"): ?><button data-url='<?php echo U("Group/del?id=".$list["id"]);?>' class="btn-sm btn btn-default delBtn" type="button">删除</button><?php endif; ?>
                                    </td>
