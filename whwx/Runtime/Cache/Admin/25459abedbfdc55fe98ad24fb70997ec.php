    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-body">
            	<label><a href="<?php echo U('TourMerchant/add');?>" class="btn btn-primary">添加</a></label>
                    ...
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                        	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                    <td><?php echo ($i); ?></td>
                                    <td><?php echo ($list["name"]); if(($list["fid"]) == "0"): ?>【WXBD#5#<?php echo ($list["phone"]); ?>#<?php echo ($list["id"]); ?>】<?php endif; ?></td>
                                    <td><?php echo ($list["phone"]); ?></td>
                                    <td>
                                        <button class='btn <?php if(($list["status"]) == "1"): ?>btn-primary<?php else: ?>btn-default<?php endif; ?> btn-sm setStatus' type="button" data-url='<?php echo U("TourMerchant/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>"><?php if(($list["status"]) == "1"): ?>启用<?php else: ?>禁用<?php endif; ?></button>
                                    </td>
                                    <td>
                                 	    <a class="btn-sm btn btn-default" href='<?php echo U("TourMerchant/edit?id=".$list["id"]);?>'>修改</a>
                                        <button data-url='<?php echo U("TourMerchant/reset?id=".$list["id"]);?>' class="btn-sm btn btn-default resetBtn" type="button">重置</button>
                                 	    <button data-url='<?php echo U("TourMerchant/del?id=".$list["id"]);?>' class="btn-sm btn btn-default delBtn" type="button">删除</button>
                                    </td>
