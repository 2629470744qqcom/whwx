...
<div id="wrapper">
    ...
    <div id="page-wrapper">
		<div id="page-inner">
	        <div class="row"> <h3 class="page-header">小区管理</h3></div>
    <div class="row">
        <div class="col-md-12"> 
            <div class="panel panel-default">
                <div class="panel-body">
                    <label><a class="btn btn-primary" href="<?php echo U('Area/add');?>">添加</a></label>
                    <form action='<?php echo U("Area/index");?>' class="form-inline text-right fr m-b-5">
              小区名称：<input type="text" name='name' value='<?php echo ($_GET['name']); ?>' class="form-control mr20 w150" >
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td>
                                            <input class="setSort" type="text" size="2" data-url="<?php echo U('Area/setSort?id='.$list['id']);?>" value="<?php echo ($list["sort"]); ?>">
                                        </td>
                                        <td>
                                            <?php switch($list["status"]): case "0": ?><button class=" btn btn-default btn-sm setStatus" type="button" data-url='<?php echo U("Area/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>">禁用</button><?php break;?>
                                                <?php case "1": ?><button class="btn btn-primary btn-sm setStatus" type="button" data-url='<?php echo U("Area/setStatus?id=".$list["id"]);?>' data-status="<?php echo ($list["status"]); ?>">启用</button><?php break; endswitch;?>
                                        </td>
                                        <td>
                                            <a class=" btn btn-default btn-sm" href="<?php echo U('Area/edit?id='.$list['id']);?>">修改</a>
                                            <button data-url='<?php echo U("Area/del?id=".$list["id"]);?>' class="btn-sm btn btn-default delBtn" type="button">删除</button>
                                        </td>
